...
WIN32_CC ?= $(WIN32_DEV_TOP)/mingw-4.7.2/bin/mingw32-gcc
...
ifeq ($(OS),Windows_NT)
  ...
  GROUPME_TARGET = libgroupme.dll
  GROUPME_DEST = "$(PROGFILES32)/Pidgin/plugins"
  GROUPME_ICONS_DEST = "$(PROGFILES32)/Pidgin/pixmaps/pidgin/protocols"
  ...
  ifeq ($(shell $(PKG_CONFIG) --exists purple-3 2>/dev/null && echo "true"),)
    ifeq ($(shell $(PKG_CONFIG) --exists purple 2>/dev/null && echo "true"),)
      GROUPME_TARGET = FAILNOPURPLE
      GROUPME_DEST =
      GROUPME_ICONS_DEST =
    else
      GROUPME_TARGET = libgroupme.so
      GROUPME_DEST = $(DESTDIR)`$(PKG_CONFIG) --variable=plugindir purple`
      GROUPME_ICONS_DEST = $(DESTDIR)`$(PKG_CONFIG) --variable=datadir purple`/pixmaps/pidgin/protocols
      LOCALEDIR = $(DESTDIR)$(shell $(PKG_CONFIG) --variable=datadir purple)/locale
    endif
  else
    GROUPME_TARGET = libgroupme3.so
    GROUPME_DEST = $(DESTDIR)`$(PKG_CONFIG) --variable=plugindir purple-3`
    GROUPME_ICONS_DEST = $(DESTDIR)`$(PKG_CONFIG) --variable=datadir purple-3`/pixmaps/pidgin/protocols
